...
function config($httpProvider, $resourceProvider, uiSelectConfig, $routeProvider) {
...
app.run(['$http', '$cookies', function($http, $cookies, editableOptions) {
...
app.controller('taskController', function ($scope, taskResource, clientResource, $uibModal, $location, $timeout, statusResource, moduleResource, urgencyResource, UserClientResource, $rootScope, usersResource) {
  ...
  function getTasks() {
    ...
    $scope.tasks = UserClientResource.query({ user_id : $rootScope.userId, task_done: 'False'});
  }
  getTasks();
  ...
  getClients = function() {
    UserClientResource.query({user_id : $rootScope.userId }, function(data){
      all = {
        id : '',
        name : 'Todos',
        lastname : '',
  ...
  $scope.newTask = function() {
    $scope.task.user = $rootScope.userId;
    var uc = {}
    var nTask = $scope.task
    taskResource.save(nTask, function(tr){
      // carga relacion usuario cliente tarea
      uc.task = tr.id
      uc.user = tr.user
      uc.client = tr.client
      UserClientResource.save(uc);
    });
...
app.controller('taskCController', function ($scope, taskResource, clientResource, $uibModal, $location, $timeout, statusResource, moduleResource, urgencyResource) {
...
app.controller('commentsController', function ($scope, taskCommentsResource, $routeParams, taskResource, 
  notificationResource, taskCommentsResource, $timeout, $location, toaster, fileUpload, $rootScope) {
...
app.controller('todoController', function ($scope, todoResource, $timeout, $routeParams, $rootScope) {
  ...
  getTodoTask = function() {
    if(taskId){
      todoResource.query({user: $rootScope.userId, task : taskId}, function(data){
        $scope.todoTasks = data;
      });
    }else{
      todoResource.query({user: $rootScope.userId, task_is_null : "True"}, function(data){
        $scope.todoTasks = data;
      });
    }
  };
...
app.controller('configController', function ($scope, configurationResource, $rootScope) {
 
  /*
  userId = $rootScope.userId
  // obtiene configuracion del usuario
  getConfiguration = function() {
    configurationResource.get({id: userId}, function(data){
      $scope.config = data;
    });
  };
  getConfiguration();
...
app.controller('messagesController', function ($scope, UserClientResource, $rootScope) {
